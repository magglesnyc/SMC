...
async function login(formData: FormData) {
  ...
  try {
    await signIn("credentials", { email: formData.get("email"), password: formData.get("password"), redirectTo: callbackUrl.startsWith("/") ? callbackUrl : "/admin" });
  } catch (e) {
  ...
  return (
    <main className="flex flex-1 items-center justify-center px-6 py-16">
      <div className="surface w-full max-w-sm rounded-3xl p-8">
        ...
        <h1 className="font-display mt-1 text-3xl font-semibold text-ink">Staff sign in</h1>
